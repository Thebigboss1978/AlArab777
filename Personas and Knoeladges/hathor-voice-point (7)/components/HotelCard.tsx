...
export const HotelCard: React.FC<Props> = ({ hotel }) => {
  // Hardcoded numbers for now as they were removed from the minimal config, 
  // but we can add them to JUDY_SETTINGS if needed.
  const primaryNumber = '+201234567890';
  const secondaryNumber = '+201098765432';
  
  const handleContact = (method: 'whatsapp' | 'phone') => {
    const number = primaryNumber.replace('+', ''); 
    
    if (method === 'whatsapp') {
      const message = `Salam! I am interested in ${hotel.name} services (Horses/Sky View).`;
    ...
    <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden hover:border-pharaonic-gold transition-all duration-300 group shadow-lg shadow-black/50">
      ...
      <div className="p-4">
        ...
        <div className="mt-3 text-[10px] text-center text-slate-500 space-y-1">
          <p>Direct Lines (24/7):</p>
          <p className="font-mono text-slate-400">{primaryNumber.replace('+20', '0')}</p>
          <p className="font-mono text-slate-400">{secondaryNumber.replace('+20', '0')}</p>
        </div>
      </div>
    </div>
  );
